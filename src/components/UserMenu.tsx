...
export const UserMenu: React.FC = () => {
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className='fixed bottom-16 left-2 w-56 bg-white dark:bg-gray-900 rounded-lg shadow-xl z-[1001] border border-gray-200/50 dark:border-gray-700/50 overflow-hidden select-none'
          >
            {/* 用户信息区域 */}
      ...
      {isSettingsOpen && (
        <>
          ...
          <motion.div
            ...
            transition={{ type: 'spring', stiffness: 400, damping: 25 }}
            className='fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white dark:bg-gray-900 rounded-xl shadow-xl z-[1001] p-6'
          >
            {/* 标题栏 */}
  ...
  return (
    <>
      ...
      {mounted && createPortal(settingsPanel, document.body)}

      {/* 使用 Portal 将修改密码面板渲染到 document.body */}
      {mounted && createPortal(changePasswordPanel, document.body)}
    </>
  );
};
